...
async def test_put_and_get(check_content, s3):
    ...
    await s3.client.delete_object(Bucket=settings.S3_BUCKET_NAME, Key=f"other/{new_file_name}")
...
async def test_del_by_prefix(check_content, s3):
    ...
    files = await s3.books.get_files_by_prefix(f"{target_prefix}/", is_content_bucket=True)
    for file in files:
        await s3.client.put_object(
            Bucket=settings.S3_BUCKET_NAME,
            Key=f"{file["key"]}",
    ...
    files_with_target_prefix = await s3.books.list_objects_by_prefix(prefix=target_prefix)
