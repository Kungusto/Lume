...
from src.schemas.books import Book, Tag, Genre, BookDataWithRels, BookDataWithRelsPrivat, GenresBook
...
class BooksRepository(BaseRepository):
    ...
    async def get_filtered_with_pagination(self, search_data, limit: int = 0, offset: int = 5): 
        query = select(self.model).limit(limit).offset(offset).filter_by(is_publicated=True).order_by(self.model.book_id)

        if search_data.book_title:
            query = query.filter(
                func.lower(BooksORM.title).contains(search_data.book_title.strip().lower())
            )
        if search_data.min_age: 
            query = query.filter(
                BooksORM.age_limit > search_data.min_age
            )
        if search_data.max_age: 
            query = query.filter(
                BooksORM.age_limit < search_data.max_age
            )
        if search_data.later_than: 
            query = query.filter(
                BooksORM.date_publicated > search_data.later_than
            )

        if search_data.earlier_than: 
            query = query.filter(
                BooksORM.date_publicated < search_data.earlier_than
            )
        
        model = await self.session.execute(query)
        results = model.scalars().all()
        return [self.schema.model_validate(result, from_attributes=True) for result in results]
