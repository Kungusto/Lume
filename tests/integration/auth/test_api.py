...
@pytest.mark.parametrize(
    ...
    [
        ("USER", "user@example.com", "John", "Smith", "johny1", "1", 422), # слишком короткий пароль
        ("USER", "user@example.com", "John", "Smith", "johny1", f"{"too_long"*10}", 422),  # слишком длинный пароль
        ("USER", "John123@example.com", "John", "Smith", "johny1", "password", 409), # почта уже зарегистрирована
        ("USER", "user@example.com", "John", "Smith", "JohnyWex", "password", 409), # ник уже занят
        ("USER", "user@example.com", "", "Smith", "johny1", "password", 422), # пустое имя
        ("USER", "user@example.com", "John", "", "johny1", "password", 422), # пустая фамилия
        ("USER", "user@example.com", "John", "Smith", "johny1", "password", 200), # status: OK
    ]
)
...
async def test_logout(ac) :  
    ...
    response_logout_not_authorized = await ac.post(
        url="/auth/logout"
    )
...
async def test_get_me(ac) :
    response_not_authentificated = await ac.get(
        url="/auth/me"
    )
...
async def test_find_user(ac):
    ...
    response_with_fake_id = await ac.get(
        url=f"auth/9999999"
    )    
