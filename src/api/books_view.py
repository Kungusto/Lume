...
async def get_page(book_id: int, page_number: int, s3: S3Dep):
...
async def get_all_books_with_pagination(
    pagination_data: PaginationDep, search_data: SearchDep, db: DBDep
):
    limit = pagination_data.per_page
    offset = (pagination_data.page - 1) * pagination_data.per_page
    return await db.books.get_filtered_with_pagination(
