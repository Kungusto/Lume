...
@pytest.mark.parametrize(
    ...
    [
        ...
        [{"page":1, "per_page":5,"later_than":"2030-01-02","earlier_than":"2000-01-01"}, 400],
...
async def test_get_book_with_cover(ac, new_publicated_book_with_cover): 
    response_search = await ac.get("/books", params={
        "page": 1,
        "per_page": 10,
...
async def test_download_book(ac2, authorized_client_new_book_with_content):
    ...
    response_download_book = await ac2.get(f"books/download/99999")
...
async def test_get_book_with_rels(ac, new_book):
    ...
    response_get_non_exist_book = await ac.get(f"books/9999")
    assert response_get_non_exist_book.status_code == 404


async def test_get_book_page(auth_new_second_user, authorized_client_new_book_with_content):
    ...
    response_get_page = await auth_new_second_user.get(f"books/{book.book_id}/page/4999")
    assert response_get_page.status_code == 404
    assert response_get_page.json()

    # пытаемся получить страницу несуществующей книги
    response_get_page = await auth_new_second_user.get(f"books/9999/page/1")
    assert response_get_page.status_code == 404
    assert response_get_page.json()


async def test_get_page_from_book_without_content(new_book, auth_new_second_user):
    response_get_page = await auth_new_second_user.get(f"books/{new_book.book_id}/page/1")
    assert response_get_page.status_code == 404
    assert response_get_page.json()
...
async def test_report_book(new_book, auth_new_user, seed_reasons):
    ...
    response_add_report = await auth_new_user.post(
        ...
        json={
            "reason_id": 9999,
            "comment": "Жалуюсь по вымышленной причине"
        }
    )
    assert response_add_report.status_code == 404
    assert response_add_report.json()
